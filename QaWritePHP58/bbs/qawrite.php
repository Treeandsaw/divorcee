...
if(is_file($skin_file)) {
    ...
    if($w == 'u' || $w == 'r') {
        ...
        $sql = " select * from {$g5['scrap_table']} where ms_id = '$qa_id' "; 

        $viewww = sql_fetch($sql);

        $sql = " select * from {$g5['poll_etc_table']} where pc_id = '$qa_id' "; 

        $viewwww = sql_fetch($sql);

        $write = $vieww + $viewww + $viewwww; 

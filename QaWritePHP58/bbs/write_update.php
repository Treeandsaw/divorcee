...
$p_image = true;
$p_thumb = apms_wr_thumbnail($bo_table, $wrt, 0, 0);
if(!$p_thumb) {
	$p_image = false;	
	$p_thumb = 1;
}
...
// 글업데이트
sql_query(" update {$write_table} set as_list = '{$as_list}', as_thumb = '".addslashes($p_thumb)."' where wr_id = '{$wr_id}' ", false);
if($is_new) sql_query(" update {$g5['board_new_table']} set as_type = '{$as_type}', as_extra = '{$as_extra}', as_list = '{$as_list}', as_secret = '{$as_secret}', as_publish = '{$as_publish}', as_update = '{$as_update}' where bo_table = '{$bo_table}' and wr_id = '{$wr_id}' ", false);
